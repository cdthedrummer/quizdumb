...
class QuizScreen extends StatelessWidget {
  ...
  Widget _buildQuestionWidget(Question question, QuizProvider provider) {
    if (question.isScale) {
      return ScaleQuestion(
        ...
        value: provider.getAnswersForQuestion(question.id)?.first != null
            ? int.parse(provider.getAnswersForQuestion(question.id)!.first)
            : 4,
    ...
    return MultipleChoiceQuestion(
      ...
      selectedAnswers: provider.getAnswersForQuestion(question.id) ?? [],
  ...
  bool _canMoveNext(QuizProvider provider) {
    ...
    return provider.getAnswersForQuestion(currentQuestionId) != null;
  }
}